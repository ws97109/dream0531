...
# 導入整合後的 FramePack 模組
from framepack_integrator import generate_dream_video, is_framepack_available
...
class DreamAnalyzer:
   ...
   def _create_directories(self):
       ...
       directories = [
           ...
           os.path.join(self.static_dir, 'videos'),
...
5. 繁體中文"""
   ...
   def _generate_video_with_settings(self, image_path, dream_text, video_length, video_quality, progress_callback=None):
       """
       使用整合的 FramePack 生成影片
       """
       if not self._check_framepack_status():
           print("❌ FramePack 不可用")
           if progress_callback:
               progress_callback("❌ FramePack 不可用")
           return None, None
       ...
       try:
           ...
           print(f"🎬 開始生成影片: {full_image_path}")
           print(f"📊 設定: {video_length}秒, {video_quality}品質")
       ...
       try:
           ...
           response = {
               ...
               'processingInfo': {
                   ...
                   'videoInfo': video_generation_info,
