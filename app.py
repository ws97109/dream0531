...
def translate_to_english(text):
    ...
    try:
        system_prompt = "你是一位翻譯專家。請將用戶輸入的任何語言翻譯成英文，只返回翻譯結果，不要包含任何額外說明。"
...
def story_to_image_prompt(story_text):
    """將故事轉換為圖像提示詞並翻譯成英文"""
    system_prompt = """將中文故事轉換為英文圖像生成提示詞，要求：
1. 提取核心視覺元素，特別注意人物和動物
2. 如果故事中有"我"、"人物"等，要在提示詞中包含 "person flying" 或 "human figure"
3. 使用簡潔英文關鍵詞
4. 限制在40個英文單詞內
5. 只返回英文提示詞，確保包含所有重要元素"""

    user_prompt = f"故事：{story_text}\n轉換為圖像提示詞："
    ...
    # 使用翻譯功能確保是英文
    translated_prompt = translate_to_english(converted_prompt.strip())
    
    if not translated_prompt:
        return None
    
    # 檢查是否包含人物元素，如果故事中有"我"但提示詞中沒有人物，則添加
    if ('我' in story_text or '自己' in story_text) and 'person' not in translated_prompt.lower() and 'human' not in translated_prompt.lower() and 'figure' not in translated_prompt.lower():
        translated_prompt = f"person flying, {translated_prompt}"
    
    base_prompt = translated_prompt
...
def analyze():
    ...
    try:
        ...
        if local_models_status:
            image_path = generate_image_fast_local(final_story, selected_model)
