...
def initialize_local_models():
    """初始化本地 Diffusers 模型"""
    global image_pipe, video_pipe, models_loaded, current_device, torch_dtype
    ...
    try:
        ...
        if torch.backends.mps.is_available():
            current_device = "mps"
            torch_dtype = torch.float32  # MPS 在某些情況下需要 float32
            logger.info("使用 Apple Silicon MPS 加速")
        elif torch.cuda.is_available():
            current_device = "cuda"
            torch_dtype = torch.float16
            logger.info(f"使用 CUDA 加速，GPU: {torch.cuda.get_device_name()}")
        else:
            current_device = "cpu"
            torch_dtype = torch.float32
            logger.info("使用 CPU 模式")
        
        logger.info(f"設備: {current_device}, 數據類型: {torch_dtype}")
        
        # 文字轉圖像模型
        logger.info("載入 Stable Diffusion 圖像生成模型...")
        ...
        if current_device != "mps":
            pipe_kwargs["variant"] = "fp16" if torch_dtype == torch.float16 else None
        
        image_pipe = StableDiffusionPipeline.from_pretrained(model_id, **pipe_kwargs)
        image_pipe = image_pipe.to(current_device)
        ...
        logger.info("✅ 圖像生成模型載入完成")
        
        # 圖像轉視頻模型
        logger.info("載入 Stable Video Diffusion 視頻生成模型...")
        try:
            ...
            if current_device != "mps" and torch_dtype == torch.float16:
                video_kwargs["variant"] = "fp16"
            
            video_pipe = StableVideoDiffusionPipeline.from_pretrained(video_model_id, **video_kwargs)
            video_pipe = video_pipe.to(current_device)
            ...
            if current_device == "cuda":
                ...
                video_pipe.enable_attention_slicing(1)
            
            logger.info("✅ 視頻生成模型載入完成")
        except Exception as video_error:
            logger.warning(f"視頻模型載入失敗: {str(video_error)}")
            video_pipe = None
...
6. 長度控制在50-80個英文單詞內
7. 加入適當的風格描述詞如 masterpiece, highly detailed 等
...
請只返回英文提示詞，不要解釋。"""
        ...
        # 清理和增強提示詞
        enhanced_prompt = clean_and_enhance_prompt(converted_prompt)
...
def clean_and_enhance_prompt(prompt):
    """清理和增強提示詞"""
    if not prompt:
        return "dreamlike scene, fantasy, beautiful, masterpiece, best quality"
    
    # 移除可能導致問題的詞彙
    forbidden_words = ["nude", "naked", "violence", "blood", "gore", "explicit"]
    words = prompt.split()
    cleaned_words = [word for word in words if not any(fw in word.lower() for fw in forbidden_words)]
    
    cleaned_prompt = " ".join(cleaned_words)
    
    # 確保有品質增強詞
    quality_words = ["masterpiece", "best quality", "highly detailed", "8k", "professional"]
    if not any(qw in cleaned_prompt.lower() for qw in quality_words):
        cleaned_prompt += ", masterpiece, best quality, highly detailed"
    
    # 確保有風格描述
    if "cinematic" not in cleaned_prompt.lower():
        cleaned_prompt += ", cinematic lighting"
    
    return cleaned_prompt[:200]  # 限制長度
...
def extract_visual_keywords(text):
    ...
    # 加入品質詞
    result = ', '.join(extracted_keywords[:8]) + ', masterpiece, best quality, highly detailed'
    return result
...
def generate_image_fast_local(story_text):
    ...
    try:
        ...
        if image_pipe is None:
            logger.error("圖像生成模型未載入")
            return create_default_image(f"error_{int(time.time())}.png", story_text)
        
        # 將故事轉換為圖像提示詞
        image_prompt = story_to_image_prompt(story_text)
        
        logger.info(f"開始生成圖像，提示詞: {image_prompt}")
        
        # 生成參數（針對速度和品質平衡）
        generation_params = {
            "prompt": image_prompt,
            "negative_prompt": "blurry, low quality, distorted, ugly, bad anatomy, deformed, watermark, signature, text, words, letters, cropped, out of frame, worst quality, low quality, jpeg artifacts, duplicate, morbid, mutilated, extra fingers, mutated hands, poorly drawn hands, poorly drawn face, mutation, deformed, blurry, dehydrated, bad anatomy, bad proportions, extra limbs, cloned face, disfigured, gross proportions, malformed limbs, missing arms, missing legs, extra arms, extra legs, fused fingers, too many fingers, long neck",
            "num_inference_steps": 25,  # 平衡品質和速度
            "guidance_scale": 7.5,
            "width": 512,
            "height": 512,
            "generator": torch.Generator(device=current_device).manual_seed(42)
        }
        
        # 針對不同設備調整參數
        if current_device == "cpu":
            generation_params["num_inference_steps"] = 20  # CPU 模式減少步數
        
        logger.info(f"生成參數: steps={generation_params['num_inference_steps']}, guidance={generation_params['guidance_scale']}")
        ...
        if not result.images or len(result.images) == 0:
            logger.error("生成結果為空")
            return create_default_image(f"empty_{int(time.time())}.png", story_text)
        ...
        if np.mean(img_array) < 5:  # 如果平均像素值太低，可能是全黑圖像
            logger.warning("檢測到可能的全黑圖像，嘗試重新生成...")
            # 使用不同的種子重新生成
            generation_params["generator"] = torch.Generator(device=current_device).manual_seed(int(time.time()))
            with torch.no_grad():
                result = image_pipe(**generation_params)
            generated_image = result.images[0]
...
def generate_video_fast_local(image_path, story_text):
    ...
    try:
        ...
        if video_pipe is None:
            logger.warning("視頻生成模型未載入，跳過視頻生成")
            return None
        
        # 載入圖像
        full_image_path = os.path.join(static_dir, image_path)
        if not os.path.exists(full_image_path):
            logger.error(f"找不到圖像文件: {full_image_path}")
            return None
        ...
        generation_time = time.time() - start_time
        logger.info(f"視頻幀生成完成，耗時: {generation_time:.2f}秒，幀數: {len(frames)}")
        
        if not frames or len(frames) == 0:
            logger.error("視頻幀生成失敗")
            return None
...
def check_local_services():
    ...
    try:
        ...
        return ollama_status, True, local_models_status
    except Exception as e:
    ...
    try:
        ...
        response = {
            ...
            'processingInfo': {
                ...
                'useLocalModels': True,
                'generationTime': {
                    'image': 'generated' if local_models_status else 'default',
                    'video': 'generated' if video_path else 'skipped'
                }
            }
        }
...
if __name__ == '__main__':
    try:
        ...
        print("🚀 本地快速生成特性:")
        if local_models_status:
            print("   • 圖像生成: 10-30 秒（512x512，25步驟）")
            print("   • 視頻生成: 30 秒-3 分鐘（1024x576，25幀）")
            print("   • 智能故事轉圖像提示詞轉換")
            print("   • 完全離線運行（首次下載後）")
            print("   • 自動設備優化（MPS/CUDA/CPU）")
            print("   • 改進的全黑圖像檢測和重新生成")
            print("   • 增強的負面提示詞過濾")
        print("   • 所有功能通過統一界面使用")
        print("   • 完整的錯誤處理和日誌記錄")
        print("   • 自動記憶體管理和清理")
        ...
        if ollama_status:
            ...
            if local_models_status:
                print("✅ 完整功能可用（包括圖像和視頻生成）")
        else:
            print("❌ 系統無法正常運行，需要至少安裝並啟動 Ollama")
